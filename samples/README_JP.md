...
- 1 エントリ = 1 キャッシュライン
- `last_ts` と free-list metadata はキーと同じラインに置く
...
- **`last_ts` も同じ行**:
  maintenance 側は追加の payload 参照なしに期限切れ判定できる
- **状態は外部配列へ**:
...
現在の sample cache entry は key/hash/timestamp/free-list metadata のみを
保持する。
...
エントリの寿命管理は `last_ts` で行う。hit と挿入成功時に timestamp が更新される。
...
現行エントリは 64 バイト 1 ラインで、キー、`cur_hash`、`last_ts`、free-list
...
query 幅は 256。datapath microbench は active entries 50% 条件で測定している。

ベンチモードと目的:

- `datapath`: round ごとに reset する microbench。`findadd_hit`, `find_hit`, `findadd_miss`, `add_only`, `add+del`, `del_bulk`, reset-per-round の `90/10` mixed を比較する。
