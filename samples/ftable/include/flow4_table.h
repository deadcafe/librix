...
uint32_t ft_flow4_table_add_entry_idx(struct ft_flow4_table *ft,
                                      uint32_t entry_idx);

void ft_flow4_table_add_entry_idx_bulk(struct ft_flow4_table *ft,
...
uint32_t ft_flow4_table_del_entry_idx(struct ft_flow4_table *ft,
                                      uint32_t entry_idx);

void ft_flow4_table_del_key_bulk(struct ft_flow4_table *ft,
                                 const struct flow4_key *keys,
                                 unsigned nb_keys,
                                 struct ft_table_result *results);
...
static inline uint32_t
ft_flow4_table_add_entry_ptr(struct ft_flow4_table *ft,
                             const struct flow4_entry *entry)
{
    return ft_flow4_table_add_entry_idx(ft,
                                        ft_flow4_table_entry_idx(ft, entry));
}

static inline void
ft_flow4_table_add_entry_ptr_bulk(struct ft_flow4_table *ft,
                                  const struct flow4_entry *const *entryv,
                                  unsigned nb_keys,
                                  struct ft_table_result *results)
{
    if (results == NULL)
    ...
    for (unsigned i = 0; i < nb_keys; i++) {
        const struct flow4_entry *entry = (entryv != NULL) ? entryv[i] : NULL;
        results[i].entry_idx = ft_flow4_table_add_entry_ptr(ft, entry);
    }
}

static inline uint32_t
ft_flow4_table_del_entry_ptr(struct ft_flow4_table *ft,
                             const struct flow4_entry *entry)
{
    return ft_flow4_table_del_entry_idx(ft,
                                        ft_flow4_table_entry_idx(ft, entry));
}

static inline void
ft_flow4_table_del_entry_idx_bulk(struct ft_flow4_table *ft,
                                  const uint32_t *entry_idxv,
                                  unsigned nb_keys,
                                  struct ft_table_result *results)
{
    ...
    for (unsigned i = 0; i < nb_keys; i++) {
        ...
        results[i].entry_idx = ft_flow4_table_del_entry_idx(ft, entry_idx);
    }
}

static inline void
ft_flow4_table_del_entry_ptr_bulk(struct ft_flow4_table *ft,
                                  const struct flow4_entry *const *entryv,
                                  unsigned nb_keys,
                                  struct ft_table_result *results)
{
    ...
    for (unsigned i = 0; i < nb_keys; i++) {
        const struct flow4_entry *entry = (entryv != NULL) ? entryv[i] : NULL;
        results[i].entry_idx = ft_flow4_table_del_entry_ptr(ft, entry);
    }
}
...
ft_flow4_table_add_entry_bulk(struct ft_flow4_table *ft,
                              ...
                              struct ft_table_result *results)
{
    ft_flow4_table_add_entry_idx_bulk(ft, entry_idxv, nb_keys, results);
}
...
ft_flow4_table_del_bulk(struct ft_flow4_table *ft,
                        ...
                        struct ft_table_result *results)
{
    ft_flow4_table_del_key_bulk(ft, keys, nb_keys, results);
}
