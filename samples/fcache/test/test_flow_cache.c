...
RIX_STATIC_ASSERT(offsetof(struct test_flow4_user, body)
                  == sizeof(struct fc_flow4_entry),
                  "test_flow4_user body must follow fc_flow4_entry directly");
...
test_flow4_init_ex_and_event_cb(void)
{
    ...
    for (unsigned i = 0; i < MAX_ENTRIES; i++) {
        ...
        if (users[i].body.last_event != 0u || users[i].body.touch != 0u)
