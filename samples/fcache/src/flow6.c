...
fc_flow6_layout_entry_ptr_(const struct fc_flow6_cache *fc, unsigned idx)
{
    if (idx == RIX_NIL)
        return NULL;
    return (struct fc_flow6_entry *)(void *)
...
#define FCG_LAYOUT_INIT_STORAGE(fc, array, stride, entry_offset)               \
    do {                                                                       \
        ...
        (fc)->pool = (struct fc_flow6_entry *)(void *)                         \
            ((fc)->pool_base + (fc)->pool_entry_offset);                       \
    } while (0)
...
_FCG_API(flow6, init_ex)(struct fc_flow6_cache *fc,
                         ...
                         const struct fc_flow6_config *cfg)
{
    struct fc_flow6_config defcfg = {
        .timeout_tsc = UINT64_C(1000000),
        .pressure_empty_slots = FC_FLOW6_DEFAULT_PRESSURE_EMPTY_SLOTS,
    };
    ...
    RIX_ASSERT((((uintptr_t)array + entry_offset)
                & (uintptr_t)(_Alignof(struct fc_flow6_entry) - 1u)) == 0u);
    ...
    fc->pressure_empty_slots = cfg->pressure_empty_slots ?
        cfg->pressure_empty_slots : FC_FLOW6_DEFAULT_PRESSURE_EMPTY_SLOTS;
    ...
    _FCG_INT(flow6, init_thresholds)(fc);
    RIX_SLIST_INIT(&fc->free_head);
    _FCG_HT(flow6, init)(&fc->ht_head, nb_bk);
    for (unsigned i = max_entries; i > 0u; i--) {
        struct fc_flow6_entry *entry = FCG_LAYOUT_ENTRY_PTR(fc, i);
