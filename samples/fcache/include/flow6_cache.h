...
{
    unsigned char *rec = (unsigned char *)fc_flow6_cache_record_ptr(fc,
                                                                    entry_idx);
    void *entryp;
    ...
    entryp = __builtin_assume_aligned(rec + fc->pool_entry_offset,
                                      _Alignof(struct fc_flow6_entry));
    return (struct fc_flow6_entry *)entryp;
}
...
fc_flow6_cache_entry_cptr(const struct fc_flow6_cache *fc, uint32_t entry_idx)
{
    const unsigned char *rec =
        (const unsigned char *)fc_flow6_cache_record_cptr(fc, entry_idx);
    const void *entryp;
    ...
    entryp = __builtin_assume_aligned(rec + fc->pool_entry_offset,
                                      _Alignof(struct fc_flow6_entry));
    return (const struct fc_flow6_entry *)entryp;
}
