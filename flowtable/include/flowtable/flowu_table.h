...
/**
 * @brief Initialise a flowu table.
 *
...
/** @brief Remove all registered flowu entries while keeping memory attached. */
void ft_flowu_table_flush(struct ft_table *ft);
/** @brief Return the current number of registered flowu entries. */
...
/**
 * @brief Look up a flowu key and optionally refresh its timestamp.
 *
...
/**
 * @brief Bulk lookup for @p nb_keys flowu keys.
 */
...
/**
 * @brief Register an already-populated flowu entry by 1-origin index.
 *
...
/**
 * @brief Bulk register already-populated flowu entries by index.
 *
...
/**
 * @brief Add flowu entries with inline registered-bucket maintenance.
 *
 * Same semantics as ft_flow4_table_add_idx_bulk_maint(), using flowu entry
 * metadata for expiry.
 */
unsigned ft_flowu_table_add_idx_bulk_maint(struct ft_table *ft,
                                           u32 *entry_idxv,
                                           unsigned nb_keys,
                                           enum ft_add_policy policy,
                                           u64 now,
...
/**
 * @brief Bulk delete by flowu key.
 *
...
/**
 * @brief Delete one registered flowu entry by index.
 *
...
/** @brief Iterate registered flowu entries in bucket order. */
...
/** @brief Const variant of ft_flowu_table_record_ptr(). */
...
/** @brief Return the embedded flowu_entry pointer for @p entry_idx. */
...
/** @brief Const variant of ft_flowu_table_entry_ptr(). */
...
/** @brief Convert an embedded flowu_entry pointer back to a 1-origin index. */
static inline u32
...
{
    u32 idx;
    return ft_flowu_table_del_key_bulk(ft, key, 1u, &idx) > 0u ? idx : 0u;
}
