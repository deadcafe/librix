...
Red-Black ツリー)、`u32` インデックス ring、高性能カッコーハッシュ
テーブルを、生ポインタを一切埋め込まない**相対インデックス**で実装します。
共有メモリや mmap 領域にそのまま格納できます。
...
include/
  ...
  rix/
    ...
    rix_ring.h      u32 インデックス FIFO/LIFO ring
...
```c
#include "librix.h"      /* ring + キュー + ツリー + 全ハッシュバリアント */
```
...
`rix_ring` は、呼び出し側が用意した `u32` 配列を使う小さなヘッダオンリーの
インデックスコンテナです。他の rix コンテナと同じ 1-origin インデックスを
想定しており、`RIX_NIL` (`0`) は格納要素として扱いません。

同じ `struct rix_ring` を、API の選択で 2 通りに使えます。

- FIFO: `rix_ring_enqueue_*()` / `rix_ring_dequeue_*()`
- LIFO: `rix_ring_push_*()` / `rix_ring_pop_*()`

主な用途は free index pool、pending index queue、共有メモリ向け allocator
の再利用可能な状態です。ring が持つのはインデックスだけで、 backing の
`u32` 配列と実レコード領域はどちらも呼び出し側が所有します。
...
```c
struct rix_ring {
    u32 *data;      /* 呼び出し側所有の storage */
    u32  capacity;  /* data[] の要素数 */
...
burst 系 API は実際に移動できたインデックス数を返します。そのため、満杯時の
部分 enqueue/push や、空に近い状態での部分 dequeue/pop を呼び出し側で扱えます。
`_seq` helper は container を reset し、連続したインデックス範囲で埋めます。
...
| mrsw/mrmw | `rix_hash_mr.h`    | fp/keyonly/u32/u64/slot_extra を generator で選択 | bucket ごとに `ctrl`; 必要に応じて `hash_field`/`slot_field`; MRMW writer lock | 128 B または 192 B、15 slots | lockless reader + single/multi writer |
...
| `-msse4.2` のみ       | **SSE4.2**     |  6.3 | XMM 128-bit — GEN 比 ×5.7 |
| `-mavx2 -msse4.2`    | SSE4.2         |  6.0 | XMM 128-bit |
| `-mavx2 -msse4.2`    | **AVX2**       |  3.3 | YMM 256-bit — SSE4.2 比 ×1.8 |
...
**まとめ:** SSE4.2 tier が最も有効なのは SSE4.2 はあるが AVX2 がない CPU
...
struct mynode {
    u8  key[16];    /* キーフィールド (可変長) */
    u32 cur_hash;   /* current-bucket hash (名前は任意) */
    u32 value;
};
...
`hash_fn` シグネチャ: `union rix_hash_hash_u hash_fn(const key_type *key, u32 mask)`。
...
RIX_HASH32 と同じインターフェースで `u64` キーに対応。
...
- SIMD を有効にする場合は、ハッシュ操作を行う各ソースファイルで
  ...
  `RIX_HASH_ARCH_SSE` を渡すと SSE4.2 XMM tier 止まり（AVX2 不使用）。
